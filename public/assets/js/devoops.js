...
function LoadAmchartsScripts(callback){
	...
	function LoadPieScript(){
		$.getScript('../assets/plugins/amcharts/pie.js', callback);
	}
	if (!$.fn.AmCharts){
		LoadAmchartsScript();
	}
...
// 	xhr.send();
// 	// var value = $(x).val();
// 	// var title = $("#"+value+"prod").val();;
// 	//
// 	// var id = $(x).attr('id');
//   //   rowCount++;
//   //   $('#prod_sec').append("<tr id='prodCount" + value + "'><td>" + title + "</td><input type='hidden' name='prod[]' value='" + value + "'><td><button type='button' class='btn btn-danger btn-app-sm btn-circle' onclick='removeprod(\"" + value + "\")'><i class='fa fa-times'></i></button></td></tr>");
// }
function addprod(x) {
	var xhr;
	if (window.XMLHttpRequest) {
...
function addimei() {
    rowCount++;
    var recRow = '<div id="imeiCount' + rowCount + '" class="row"><div class="col-md-10"><input name="imei[]" id="id' + rowCount + '" type="text" class="form-control" /></div><div class="col-md-2"><button type="button" class="btn btn-danger btn-app-sm btn-circle" onclick="removeimei(' + rowCount + ');"><i class="fa fa-times"></i></button></div></div>';
    jQuery('#imei_sec').append(recRow);
}
function removeimei(removeNum) {
    //del(removeNum);
    jQuery('#imeiCount' + removeNum).remove();
}
function addProdR(){
	rowCount++;
	var recRow = '<div id="prodCount' + rowCount + '" class="row"><div class="col-md-5"><label class="col-md-12">Product<label><input name="product[]" id="id' + rowCount + '" type="text" class="form-control" /></div><div class="col-md-5"><label class="col-md-12">Issue<label><textarea name="issue[]" id="id' + rowCount + '" class="form-control"></textarea></div><div class="col-md-2"><button type="button" class="btn btn-danger btn-app-sm btn-circle" onclick="removeProdR(' + rowCount + ');"><i class="fa fa-times"></i></button></div><hr/></div>';
	jQuery('#Prod-Repair').append(recRow);
}
function removeProdR(removeNum) {
    jQuery('#prodCount' + removeNum).remove();
}
function addcoll(x){
     var values1 = $(x).val();
    ...
    //  $('#addCollected').append("<div class='col-md-12' id='col_" + values1 + "'><div class='col-md-5'><input type='text' value='" + id1 + "' disabled class='form-control'></div><input type='hidden' name='col[]' value='" + values1 + "'><div class='col-md-5'><input type='number' name='col_qty_" + values1 + "[]' value='1' placeholder='Number Of Products' class='form-control'></div><div class='col-md-2'><button type='button' class='btn btn-danger btn-app-sm btn-circle' onclick='removecol(this, \"" + values1 + "\")'><i class='fa fa-times'></i></button></div></div>");
		$('#addCollected').append("<div class='col-md-12' id='col_" + values1 + "'><div class='col-md-10'><input type='text' value='" + id1 + "' disabled class='form-control'></div><input type='hidden' name='col[]' value='" + values1 + "'><div class='col-md-2'><button type='button' class='btn btn-danger btn-app-sm btn-circle' onclick='removecol(this, \"" + values1 + "\")'><i class='fa fa-times'></i></button></div></div>");
     //$(x).parent().remove();
 }
...
jQuery('#col_' + val).remove();
		 //$(values2).parent().remove();
}

function sale_addImei(id) {
		var xhr;
		if (window.XMLHttpRequest) {
				xhr = new XMLHttpRequest();
		}
		...
		xhr.open('GET', '/sale/add_imei/'+id);
		xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		xhr.send();
	}

	function sale_editImei(id) {
			var xhr;
			if (window.XMLHttpRequest) {
					xhr = new XMLHttpRequest();
			}
			...
			xhr.open('GET', '/sale/edit_imei/'+id);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			xhr.send();
		}
	...
	function sale_edit(id) {
		...
		function sale_status(id) {
							...
							if (xhr.readyState === 4) {
									if (xhr.status == 200 && xhr.status < 300) {
										$('#repair-Modal').modal('show');
											document.getElementById('repairBody').innerHTML = xhr.responseText;
									}
							}
					}
					xhr.open('GET', '/repair/repair_payment/'+id);
					xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
					xhr.send();
				}
			function vendor_imei_edit(id) {
					var xhr;
					if (window.XMLHttpRequest) {
							xhr = new XMLHttpRequest();
					}
					...
					xhr.open('GET', '/vendor/collected/edit/'+id);
					xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
					xhr.send();
				}
				function vendor_imei_status(id) {
						var xhr;
						if (window.XMLHttpRequest) {
								xhr = new XMLHttpRequest();
						}
						...
						xhr.open('GET', '/vendor/collected/status/'+id);
						xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
						xhr.send();
					}
					function edit_log(id) {
						...
						function client_data(id) {
							...
							function repair_edit(id) {
									...
									xhr.open('GET', '/repair/edit/'+id);
									xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
									xhr.send();
								}
								function repairp_edit(id) {
										var xhr;
										if (window.XMLHttpRequest) {
												xhr = new XMLHttpRequest();
										}
										...
										xhr.open('GET', '/repair/edit_product/'+id);
										xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
										xhr.send();
									}

			$('#inquiry-status').on('change', function() {
				var status =$('select[name="inquiry-status"]').val();
				...
				xhr.open('POST', '/inquiry/inquiry_status/');
				xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				xhr.send(sent);
			});
			...
			//Search The clients
			$("#search_client").keydown(function(){
			...
			$("#h_search_client").keydown(function(){
...
$(document).ready(function () {
	// $('[name="col_vendor"]').change(function() {
	// 		var val = $(this).val();
	// 		var xhr;
	...
	// 				if (xhr.readyState === 4) {
	// 						if (xhr.status == 200 && xhr.status < 300) {
	// 								document.getElementById('vendor_table').innerHTML = xhr.responseText;
	// 								document.getElementById('addCollected').innerHTML = '';
	// 						}
	// 				}
	// 		}
	// 		xhr.open('GET', '/sale/col_vendor/'+val);
	// 		xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	// 		xhr.send();
	// 	});
